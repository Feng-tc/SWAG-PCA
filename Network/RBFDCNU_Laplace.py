...
class NuNetLaplace(GenerativeRegistrationNetwork):
    def __init__(self,
                 encoder_param,
                 i_size,
                 c_factor,
        ...
        super(NuNetLaplace, self).__init__(i_size)
    ...
    def Laplace_test(self, src, tgt, nucp_loc, k, s):
        ...
        for _ in range(s):
            ...
            nucp_loc = nucp_loc - self.gap
            uzMean, _, nuzMean, _ = self.encoder(src, tgt, nucp_loc, ucp_loc)
            nucp_loc = nucp_loc + self.gap
            ...
            lst_flow.append(flow.unsqueeze(0))

        flow = torch.cat(lst_flow, dim=0)

        self.nudecoder.del_weight()

        return flow.mean(0), flow

    # def sampling_theta(self, k):
    #     theta = getattr(self, 'cap_theta')
    #     b = getattr(self, 'cap_b')
    #     cov_diag = b - theta ** 2
    #     cov_diag = torch.clamp(cov_diag, min=0)
    #     D = getattr(self, 'cap_D')

    #     z1 = torch.randn_like(theta)
    #     z2 = torch.randn((k, 1), device=theta.device)

    #     theta_apx = theta + (0.5 * cov_diag) ** 0.5 * z1 + (torch.matmul(D.T, z2).squeeze(-1) / (2 * (k - 1)) ** 0.5)
    ...
    def sampling_theta(self, k):
        ...
        u = torch.rand_like(theta)  # U[0,1)
        u = torch.clamp(u, min=1e-3)
        abs_term = 2 * torch.abs(u - 0.5)
        # abs_term = torch.clamp(abs_term, max=1 - 1e-10, min=1e-10)
        log_input = 1 - abs_term

        # 符号函数
        sign_term = torch.where(u > 0.5, 1.0, -1.0)

        # 生成Laplace噪声
        base_noise = -b * sign_term * torch.log(log_input)

        theta_apx = theta + base_noise
