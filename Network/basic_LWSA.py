...
class LWSA(nn.Module):
    def __init__(self):
        ...
        self.c1 = Conv2dReLU(1, embed_dim//2, 3, 1, use_batchnorm=False)
        self.c2 = Conv2dReLU(1, 16, 3, 1, use_batchnorm=False)
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, rpe=True, attn_drop=0., proj_drop=0.):
        ...
        # define a parameter table of relative position bias
        self.relative_position_bias_table = nn.Parameter(
            torch.clamp(torch.normal(0, .02, ((2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads)), min=-2, max=2))  # 2*Wh-1 * 2*Ww-1 * 2*Wt-1, nH

        # get pair-wise relative position index for each token inside the window
        coords_h = torch.arange(self.window_size[0])
        coords_w = torch.arange(self.window_size[1])
        coords = torch.stack(torch.meshgrid([coords_h, coords_w]))  # 3, Wh, Ww, Wt
        coords_flatten = torch.flatten(coords, 1)  # 3, Wh*Ww*Wt
        self.rpe = rpe
        if self.rpe:
            relative_coords = coords_flatten[:, :, None] - coords_flatten[:, None, :]  # 3, Wh*Ww*Wt, Wh*Ww*Wt
    ...
    def forward(self, x, mask=None):
        ...
        Args:
            ...
            mask: (0/-inf) mask with shape of (num_windows, Wh*Ww, Wh*Ww, Wt*Ww) or None
        """
        B_, N, C = x.shape # [B * 窗口数, 单个窗口H * 单个窗口W, D]
        # self.qkv(x): [B * 窗口数, 单个窗口H * 单个窗口W, D] -> [B * 窗口数, 单个窗口H * 单个窗口W, 3 * 头数 * Dh], 这里Dh=24
        # [3代表qkv3个向量, B * 窗口数, 头数, 单个窗口H * 单个窗口W, D / 头数]
        qkv = self.qkv(x).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4) 
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(7, 7, 7), shift_size=(0, 0, 0),
        ...
        self.attn = WindowAttention(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop) # 和原WindowAttention相比，没有rpe参数

        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity() # 和原SwinTransformerBlock相比，多了这行
    ...
    def forward(self, x, mask_matrix):
        ...
        attn_windows = attn_windows.view(-1, self.window_size[0], self.window_size[1], C) # [B * 窗口数, 窗口H, 窗口W, C]
        shifted_x = window_reverse(attn_windows, self.window_size, Hp, Wp)  # B H' W' L' C
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        mask_windows = window_partition(img_mask, self.window_size)  # [窗口数，单个窗口H, 单个窗口W, 1]
