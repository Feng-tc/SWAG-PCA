...
def LoadCheckpoint(net, model_save_path, epoch=None, verbose=1):
    ...
    if type(net) == dict:
        ...
        # model_saved.pop('nudecoder.weight')
        net.load_state_dict(model_saved)
        net.eval()

def SaveCheckpoint(net, model_save_path, epoch, verbose=None, optimizer=None):
    if not os.path.exists(model_save_path):
        os.mkdir(model_save_path)

    if type(net) == dict:
        for key in net:
            save_checkpoint_path = os.path.join(model_save_path, '{}.{}.pt'.format(key, epoch))
            torch.save(net[key].state_dict(), save_checkpoint_path)
    else:
        save_checkpoint_path = os.path.join(model_save_path, '{}.pt'.format(epoch))
        torch.save(net.state_dict(), save_checkpoint_path)
        if optimizer != None:
            save_optimizer_path = os.path.join(model_save_path, '{}_{}.pt'.format('optimizer', epoch))
