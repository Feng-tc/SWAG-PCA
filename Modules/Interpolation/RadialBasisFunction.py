...
class RadialBasisLayer_MultiSamples(nn.Module):

    def __init__(self, i_size, c_factor=2, cpoint_num=160):

        super(RadialBasisLayer_MultiSamples, self).__init__()
        ...
        loc = torch.stack(loc, 2)
        loc = loc[:, :, [1, 0]].float().unsqueeze(2)
        # 将每个像素的坐标重复cpoint_num次
        loc_tile = loc.repeat(1, 1, cpoint_num, 1) # (H, W, cpoint_num, 2)
        self.register_buffer('loc_tile', loc_tile)

    def get_weight(self, cpoint_loc):
        # 计算compact support
        c = self.cpoint_maxmin(cpoint_loc)[0]
        c = (c * self.c_factor).view(-1, 1, 1, 1)

        # 直接在计算距离时添加batch维度，避免多次unsqueeze
        batch_size = cpoint_loc.size(0)
        loc_tile = self.loc_tile.expand(batch_size, *self.loc_tile.shape)
        
        # 重塑控制点位置，减少repeat操作
        cp_loc = cpoint_loc.view(batch_size, 1, 1, -1, 2)
        
        # 计算平方距离
        diff = loc_tile - cp_loc
        dist_sq = torch.sum(diff * diff, dim=-1) / (c * c)
        ...
        dist[mask] = torch.sqrt(dist_sq[mask])
        
        # 使用torch.where优化mask操作
        weight = torch.where(
            mask,
            torch.pow(1 - dist, 4) * (4 * dist + 1),
    ...
    def forward(self, alpha):
        ...
        alpha = alpha.unsqueeze(2).unsqueeze(2)
        phi = torch.sum(self.weight * alpha, -2)
        phi = phi.permute(0, 1, 4, 2, 3)
        return phi


class RadialBasisLayer_MultiSamples_G(nn.Module):
    def __init__(self, cp_loc, i_size, c_factor=2, cpoint_num=160):
        super(RadialBasisLayer_MultiSamples_G, self).__init__()

        # 直接在GPU上创建坐标网格，使用arange代替linspace提高效率
        y_coords = torch.arange(i_size[0], device='cuda', dtype=torch.float32)
        x_coords = torch.arange(i_size[1], device='cuda', dtype=torch.float32)
        
        # 使用新版本meshgrid，避免后续的转置操作
        grid_y, grid_x = torch.meshgrid(y_coords, x_coords, indexing='ij')
        loc = torch.stack([grid_x, grid_y], dim=-1).unsqueeze(2)
        
        # 使用expand代替repeat来节省内存
        loc_tile = loc.expand(-1, -1, cpoint_num, -1)
        
        # 计算支撑域，优化维度操作
        cpoint_maxmin = MaxMinPointDist(cpoint_num)
        c = cpoint_maxmin(cp_loc)[0] * c_factor
        c = c.view(-1, 1, 1, 1)
        
        # 优化控制点位置的处理
        cp_loc = cp_loc.view(-1, 1, 1, cpoint_num, 2)
        
        # 计算平方距离，避免直接使用norm
        diff = loc_tile.unsqueeze(0) - cp_loc
        dist_sq = torch.sum(diff * diff, dim=-1) / (c * c)
        
        # 使用mask优化计算
        mask = dist_sq < 1.0
        weight = torch.zeros_like(dist_sq)
        
        # 只对需要的部分计算权重
        if mask.any():
            valid_dist = torch.sqrt_(dist_sq[mask])  # 使用inplace操作
            valid_weight = (1 - valid_dist).pow_(4)
            valid_weight.mul_(4 * valid_dist + 1)
            weight[mask] = valid_weight
...
class RadialBasisLayer_Phi_Var(nn.Module):
    def __init__(self, i_size, c_factor=2, cpoint_num=160):
        super(RadialBasisLayer_Phi_Var, self).__init__()
        # a location mesh of output
        loc_vectors = [torch.linspace(0.0, i_s - 1, i_s) for i_s in i_size]
        loc = torch.meshgrid(loc_vectors)
        loc = torch.stack(loc, 2)
