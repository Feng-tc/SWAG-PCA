...
class MaxMinPointDist(torch.nn.Module):
    def __init__(self, point_num, max_v=1.0e4):
        super(MaxMinPointDist, self).__init__()
        self.mask = torch.tensor([8]).cuda()

        self.point_num = point_num
        bigeye = torch.eye(point_num, point_num) * max_v
        self.register_buffer('bigeye', bigeye.cuda())
        
    def forward(self, point):

        point1 = point.unsqueeze(1).repeat(1, self.point_num, 1, 1)
        point2 = point.unsqueeze(2).repeat(1, 1, self.point_num, 1)

        dist = torch.norm(point1 - point2, dim=3)

        min_dist = torch.min(dist + self.bigeye, dim=2)[0]

        '''
            r = 2 max mim ||p_i - p_j||，其结论仅对均匀布点有效。
        '''
        c = torch.max(min_dist, dim=1)[0]
        c = torch.max(c, self.mask)
